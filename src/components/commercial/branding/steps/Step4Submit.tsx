...
interface Props {
  ...
  onBack: () => void;
  onSubmit: (designerNote: string) => void;
}
...
export const Step4Submit: React.FC<Props> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
      ...
      {/* Comentario inicial para el diseñador */}
      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">
          Comentario para el diseñador{' '}
          <span className="text-gray-400 font-normal">(opcional)</span>
        </label>
        <p className="text-xs text-gray-500">
          Déjale instrucciones o contexto inicial al diseñador que tomará tu solicitud.
        </p>
        <textarea
          value={note}
          onChange={e => setNote(e.target.value.slice(0, MAX_NOTE))}
          rows={3}
          placeholder="Ej: El logo principal es el azul marino. Priorizar colores corporativos sobre el resto..."
          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
        />
        <div className="flex justify-end">
          <span className="text-xs text-gray-400">{note.length}/{MAX_NOTE}</span>
        </div>
      </div>
