...
import { getMyPromoters } from "@/services/AlliesService";
...
export default function CommercialProfileSeller() {
  ...
  const [promoters, setPromoters] = useState<AllyCommercialResponseDTO[]>([]);
  ...
  useEffect(() => {
    const loadPromoters = async () => {
      try {
        setPromoters(await getMyPromoters());
      } catch {
        // silencioso: la sección de aliados no bloquea el resto del perfil
      } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-14">
      ...
      {/* ── ALIADOS QUE TE PROMOCIONAN ── */}
      <AlliesSummarySection
        title="Comerciales que te promocionan"
        description="Cuentas Premium que están promocionando tus productos en el pop up final de sus juegos."
        allies={promoters}
        loading={promotersLoading}
        emptyText="Todavía ningún comercial Premium te ha agregado como aliado."
      />
            ...
            {hasMore && (
              <div className="flex justify-center mt-8">
                <button
                  ...
                  className="px-8 py-2.5 rounded-xl bg-[#03548C] text-white font-semibold text-sm hover:bg-[#0b1440] transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
