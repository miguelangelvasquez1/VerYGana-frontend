...
function TabBtn({ active, onClick, icon, label }: {
  ...
  return (
    <button
      onClick={onClick}
      className={`cursor-pointer flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium transition-all ${
...
function ExportBtn({ label, icon, loading, disabled, onClick, primary = false }: {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`cursor-pointer inline-flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-sm font-semibold transition-all disabled:opacity-50 active:scale-95 ${
        primary
