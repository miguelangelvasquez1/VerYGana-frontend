...
import {
  getAdminAuditLogs,
  getCriticalAdminAuditLogs,
  type AdminAuditLogFilters,
  type AdminAuditLogsResult,
} from '@/services/admin/AdminAuditLogsService';
...
export default function OtherAuditEventsPanel() {
  ...
  const [data, setData] = useState<AdminAuditLogsResult | null>(null);
  ...
  const fetchEvents = useCallback(async (
    ...
    activeFilters: AdminAuditLogFilters,
    page: number
  ) => {
    setLoading(true);
    setError(false);
    try {
      ...
      if (activeTab === 'critical') {
        setData(await getCriticalAdminAuditLogs(activeFilters.category || undefined, from, to, page, PAGE_SIZE));
      } else {
        setData(await getAdminAuditLogs({ ...activeFilters, from, to, page, size: PAGE_SIZE }));
      }
    } catch {
      setError(true);
    } finally {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Otros eventos</h2>
          <p className="text-xs text-gray-500 mt-0.5">
            Eventos WARNING/CRITICAL fuera de seguridad (ej. PQRS) — complementa al panel de seguridad, sin solaparse.
          </p>
        </div>
        <button
        ...
        {loading ? (
          ...
          summaryCards.map(([action, count]) => (
            <div key={action} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:border-gray-200 hover:shadow-md transition-all">
              ...
              <p className="text-xs text-gray-500 mt-1 truncate" title={action}>
                {action}
              </p>
            </div>
          ))
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          ...
          {tab === 'critical' && (
            <p className="text-xs text-gray-400 mt-2">
              El tab &quot;Solo Críticos&quot; ya filtra por nivel CRITICAL — el filtro de acción y nivel no aplica aquí (categoría sí).
            </p>
          )}
                    ...
                    return (
                      <Fragment key={ev.id}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-medium text-gray-800">{ev.action}</td>
                          ...
                          <td className="px-4 py-3 text-gray-600">
                            {ev.username ? (
                              <div className="flex flex-col">
                                ...
                                {ev.userId != null && (
                                  <span className="text-[10px] text-gray-400 font-mono">#{ev.userId}</span>
                                )}
                              </div>
                            ) : (
