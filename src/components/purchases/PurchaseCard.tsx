...
const statusColor: Record<PurchaseStatus, string> = {
  [PurchaseStatus.PENDING]: "text-amber-600 bg-amber-50",
  [PurchaseStatus.PROCCESSING]: "text-[#03548C] bg-[#03548C]/10",
...
const statusDot: Record<PurchaseStatus, string> = {
  [PurchaseStatus.PENDING]: "bg-amber-500",
  [PurchaseStatus.PROCCESSING]: "bg-[#03548C]",
  [PurchaseStatus.COMPLETED]: "bg-green-500",
  [PurchaseStatus.FAILED]: "bg-red-500",
};
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow overflow-hidden">
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <div className="flex justify-between items-start gap-3">
          ...
          <span
            className={`inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold px-2.5 py-1 rounded-full shrink-0 ${statusColor[purchase.status]}`}
          >
            <span className={`w-1.5 h-1.5 rounded-full ${statusDot[purchase.status]}`} />
        ...
        {/* Resumen financiero */}
        <div className="border-t border-gray-100 pt-3 space-y-2 text-sm sm:text-base">
          {hasKeys && (
          ...
          <div className="flex justify-between items-center font-semibold text-gray-900">
            ...
            <span className="text-base sm:text-lg font-bold text-[#03548C]">${formatPesos(purchase.cashCents)}</span>
          </div>
        </div>
